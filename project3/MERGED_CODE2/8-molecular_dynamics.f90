subroutine molecular_dynamics(Natoms, coord, velocity, acceleration, mass, epsilon, sigma, distance)
    ...
    do step = 1, total_steps
        ...
        coord = coord + (velocity * dt) * 1000 + (0.5d0 * acceleration * dt**2) / 1000

        ! 2. Compute velocities part dependent on a(n) (v(n+**) = v(n) + 0.5*a(n)*dt)
        temp_velocity = velocity + (0.5d0 * acceleration * dt) / 1.0d6
        ...
        velocity = temp_velocity + (0.5d0 * acceleration * dt) / 1.0d6
        ...
        if (mod(step, M) == 0) then 
            write(10, '(A, I6, A, ES16.8, A, ES16.8, A, ES16.8)') &
                "Step ", step, ": KE=", kinetic_energy, ", PE=", &
