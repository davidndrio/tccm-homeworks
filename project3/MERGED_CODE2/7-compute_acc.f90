subroutine compute_acc(Natoms, coord, mass, distance, acceleration, epsilon, sigma)
  ...
  do i = 1, Natoms
    inv_mass = (1.0d0 / mass(i)) * 1000.0d0
    do j = 1, Natoms
      if (i /= j) then
        ...
        rij = distance(i,j)
        U = 24.0d0 * 1.0d9 * epsilon * ( (sigma/rij)**6 - 2.0d0*(sigma/rij)**12 ) / rij

        acceleration(i,1) = acceleration(i,1) - U * (dx/rij) * inv_mass
        acceleration(i,2) = acceleration(i,2) - U * (dy/rij) * inv_mass
        acceleration(i,3) = acceleration(i,3) - U * (dz/rij) * inv_mass
      end if
    end do
  end do
